	...
	for ( ; *s1; ++s1, ++s2 )
	{
		if ( *s1 != *s2 )
		{
			...
			if ( c1 != c2 || (unsigned char)(c1 - 'a') > ('z' - 'a') )
			{
				...
				if ( (c1 | c2) >= 0x80 ) return _stricmp( (const char*)s1, (const char*)s2 );
...
char *V_pretifymem( float value, int digitsafterdecimal /*= 2*/, bool usebinaryonek /*= false*/ )
{
	...
	if ( fabsf( value - (int)value ) < 0.00001 )
	{
...
bool Q_RemoveAllEvilCharacters( char *pch )
{
	...
	wchar_t *pwch = (wchar_t *)_alloca( cubDest );
...
bool Q_StripPrecedingAndTrailingWhitespaceW( wchar_t *pwch )
{
	...
	// duplicate on stack
	int cubDest = ( cch + 1 ) * sizeof( wchar_t );
	wchar_t *pwchT = (wchar_t *)_alloca( cubDest );
...
//			maxoutputbytes - 
//-----------------------------------------------------------------------------
void V_hextobinary( char const *in, int numchars, unsigned char *out, int maxoutputbytes )
{
	int len = V_strlen( in );
...
bool BGetLocalFormattedDateAndTime( time_t timeVal, char *pchDate, int cubDate, char *pchTime, int cubTime )
{
	...
	if ( timeVal )
	{
		...
		struct tm tmToDisplay = *( localtime( ( const time_t* )&timeVal ) );
#ifdef POSIX
		if ( pchDate != NULL )
		{
			pchDate[ 0 ] = 0;
